...
def compute_saliency_maps(X, y, model):
    ...
    N = X.shape[0]
    loss = torch.sum(scores[torch.arange(N), y])
    loss.backward()
    saliency = X.grad.data.abs().amax(dim=1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        max_score, label = torch.max(scores, dim=1)
        loss = scores[0][target_y]
        if verbose:
            print("iter %d , target score %.2f, max score %.2f, label %d" % (i, loss, max_score, label))
        if label == target_y:
            break
        loss.backward()
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    loss = target_score - l2_reg * torch.sum(img.data ** 2)
    loss.backward()

    img.data += img.grad.data * learning_rate
    img.grad.zero_()
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
