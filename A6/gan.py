...
def sample_noise(batch_size, noise_dim, dtype=torch.float, device="cuda"):
    """
    ...
    noise = torch.rand(size=(batch_size, noise_dim), dtype=dtype, device=device)
    noise = noise * 2 - 1
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
...
def discriminator_loss(logits_real, logits_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    real = torch.ones_like(logits_real, device=device, dtype=dtype)
    fake = torch.zeros_like(logits_fake, device=device, dtype=dtype)
    loss = torch.nn.functional.binary_cross_entropy_with_logits(logits_fake, fake) + torch.nn.functional.binary_cross_entropy_with_logits(logits_real, real)
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def generator_loss(logits_fake):
    ...
    loss = nn.functional.binary_cross_entropy_with_logits(logits_fake, fake)
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def get_optimizer(model: nn.Sequential):
    """
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        ...
        nn.ReLU(),
        nn.BatchNorm1d(1024),
        nn.Linear(1024, 49 * 128),
        nn.ReLU(),
        nn.BatchNorm1d(49 * 128),
