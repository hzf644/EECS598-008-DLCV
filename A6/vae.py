...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        self.encoder = nn.Sequential(nn.Linear(self.input_size + self.num_classes, self.hidden_dim),
                                     nn.ReLU(),
                                     nn.Linear(self.hidden_dim, self.hidden_dim),
                                     nn.ReLU(),
                                     nn.Linear(self.hidden_dim, self.hidden_dim),
    ...
    def forward(self, x, c):
        ...
        con = torch.concatenate((x, c), dim=1)
        encode = self.encoder(con)
        mu = self.mu_layer(encode)
        logvar = self.logvar_layer(encode)
        z = reparametrize(mu, logvar)
        con2 = torch.concatenate((z, c), dim=1)
        x_hat = self.decoder(con2)
        ###########################################################################
        #                                      END OF YOUR CODE                   #
        ###########################################################################
        return x_hat, mu, logvar
...
def reparametrize(mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    epsilon = torch.normal(0, 1, mu.shape, dtype=mu.dtype, device=mu.device)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    expect = F.binary_cross_entropy(x_hat, x, reduction='sum')
    loss = (kl_divergence + expect) / N
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
