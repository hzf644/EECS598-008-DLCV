...
def gram_matrix(features, normalize=True):
    ...
    trans = features.permute(0, 2, 1)
    gram = features.bmm(trans)
    if normalize:
        gram /= (H * W * C)
    ############################################################################
    #                               END OF YOUR CODE                           #
...
def tv_loss(img, tv_weight):
    ...
    left = img[:, :, :, 0:W - 1]
    right = img[:, :, :, 1:W]
    return tv_weight * (torch.sum((up - down) ** 2) + torch.sum((left - right) ** 2))
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_gram_matrix(features, masks, normalize=True):
    ...
    masks = masks.unsqueeze(dim=2).repeat(1, 1, C, 1, 1)
    features *= masks
    features = torch.flatten(features, 3)
    guided_gram = features @ (features.permute(0, 1, 3, 2))
    if normalize:
        guided_gram /= (C * H * W)
    return guided_gram
    ##############################################################################
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    loss = 0
    for i in range(len(style_layers)):
        gram = guided_gram_matrix(feats[style_layers[i]], content_masks[style_layers[i]])
        loss += style_weights[i] * torch.sum((gram - style_targets[i]) ** 2)
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
