...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        length = len(stem_channels)
        for i in range(length):
            ConvLayer1, ConvLayer2 = None, None
            if i == 0:
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key in feats_per_fpn_level:
            feat = feats_per_fpn_level[key]
            cls = self.stem_cls.forward(feat)
            box = self.stem_box.forward(feat)
            cls = self.pred_cls(cls)
            ctr = self.pred_ctr(box)
            box = self.pred_box(box)
            cls = cls.permute(0, 2, 3, 1)
            box = box.permute(0, 2, 3, 1)
            ctr = ctr.permute(0, 2, 3, 1)
            cls = cls.reshape(cls.shape[0], -1, self.num_classes)
            box = box.reshape(box.shape[0], -1, 4)
            ctr = ctr.reshape(ctr.shape[0], -1, 1)
            class_logits[key] = cls
            boxreg_deltas[key] = box
            centerness_logits[key] = ctr
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    temp_location = torch.cat((locations, -locations), dim=1)
    temp_box = gt_boxes[:, :4].clone()
    temp_box[:, 2] *= -1
    temp_box[:, 3] *= -1
    deltas = temp_location - temp_box
    deltas /= stride
...
def fcos_apply_deltas_to_locations(
    ...
    temp = deltas * stride
    output_boxes = torch.cat((locations, locations), dim=1)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    right = deltas[:, 2]
    bottom = deltas[:, 3]
    mask = (left == -1) * (right == -1) * (top == -1) * (bottom == -1)
    lr = torch.stack((left, right), dim=1)
    tb = torch.stack((top, bottom), dim=1)
    centerness = torch.sqrt((lr.amin(dim=1) * tb.amin(dim=1)) / (lr.amax(dim=1) * tb.amax(dim=1)))
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.bakcbone = DetectorBackboneWithFPN(out_channels=fpn_channels)
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        fpn_feats = self.bakcbone.forward(images)
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        shapes_per_fpn_feat = {key: fpn_feats[key].shape for key in fpn_feats}
        strides_per_fpn_feat = {'p3': 8, 'p4': 16, 'p5': 32}
        locations_per_fpn_level = get_fpn_location_coords(shapes_per_fpn_feat, strides_per_fpn_feat)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for i in range(batch_size):
            matched_gt_boxes.append(fcos_match_locations_to_gt(locations_per_fpn_level, strides_per_fpn_feat, gt_boxes[i]))
        for i in range(batch_size):
            matched_gt_deltas_singe = {key: fcos_get_deltas_from_locations(locations_per_fpn_level[key], matched_gt_boxes[i][key], strides_per_fpn_feat[key]) for key in locations_per_fpn_level}
            matched_gt_deltas.append(matched_gt_deltas_singe)
        ...
        one_hot_index = torch.cat((torch.zeros(1, self.num_classes), torch.eye(self.num_classes)), dim=0).to(
        ...
        gt_centerness = torch.zeros(batch_size, matched_gt_deltas.shape[1])
        for i in range(batch_size):
            gt_centerness[i] = fcos_make_centerness_targets(matched_gt_deltas[i])
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            ##################################################################
            # Feel free to delete this line: (but keep variable names same)
            _, _, max_x, max_y = images.shape
            level_deltas = level_deltas.reshape(-1, 4)
            level_cls_logits = level_cls_logits.reshape(-1, self.num_classes)
            level_ctr_logits = level_ctr_logits.reshape(-1)
            boxes = fcos_apply_deltas_to_locations(level_deltas, level_locations, self.bakcbone.fpn_strides[level_name])
            boxes[:, 0] = boxes[:, 0].clamp(min=0)
            boxes[:, 1] = boxes[:, 1].clamp(min=0)
            boxes[:, 2] = boxes[:, 2].clamp(max=max_x)
            boxes[:, 3] = boxes[:, 3].clamp(max=max_y)
            level_cls_logits, classes = level_cls_logits.max(dim=1)
            scores = torch.sqrt(level_cls_logits.sigmoid_() * level_ctr_logits.sigmoid_())
            mask = (scores > test_score_thresh)
            boxes = boxes[mask]
            ...
            level_pred_boxes, level_pred_classes, level_pred_scores = (
                boxes,
                classes,
                scores,  # Need tensors of shape: (N, 4) (N, ) (N, )
            )
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
