...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        ...
        for j in range(K):
            weight[j] = (query[i] * key[j]).sum()
        weight = (weight / sqrt(M)).softmax(dim=0)
        vector = (weight.view(-1, 1) * value).sum(dim=0)
        out[i] = vector
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_batch(
    ...
    for i in range(K):
        ...
        for j in range(K):
            weight[:, j] = (query[:, i] * key[:, j]).sum(dim=1)
        weight = (weight / sqrt(M)).softmax(dim=1)
        vector = (weight.view(N, K, 1) * value).sum(dim=1)
        out[:, i] = vector
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        weights[mask] = -1e9
    # Replace "pass" statement with your code
    weights_softmax = (weights / sqrt(M)).softmax(dim=2)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        multihead = [SelfAttention(dim_in, dim_out, dim_out)] * num_heads
        self.multihead = nn.ModuleList(multihead)
        self.affine = nn.Linear(dim_out * num_heads, dim_in)
        c = sqrt(6/(dim_in + dim_out * num_heads))
        nn.init.uniform_(self.affine.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        y_list = [head.forward(query, key, value, mask) for head in self.multihead]
        y_cat = torch.concatenate(y_list, dim=-1)
        y = self.affine.forward(y_cat)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        c = sqrt(6/(inp_dim + hidden_dim_feedforward))
        self.l1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        self.l2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        torch.nn.init.uniform_(self.l1.weight, -c, c)
        torch.nn.init.uniform_(self.l2.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multiHead = MultiHeadAttention(num_heads, emb_dim, int(emb_dim/num_heads))
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1 = self.multiHead(x, x, x)  # (N, K, M)
        out2 = self.dropout(self.layerNorm1(x + out1))
        out3 = self.feedForward(out2)
        y = self.dropout(self.layerNorm2(out3 + out2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        out2 = self.dropout.forward(self.norm1.forward(out1 + dec_inp))
        out3 = self.attention_cross.forward(out2, enc_inp, enc_inp)
        out4 = self.dropout.forward(self.norm2.forward(out3 + out2))
        out5 = self.feed_forward.forward(out4)
        y = self.dropout.forward(self.norm3(out5 + out4))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    array = torch.arange(K) / K
    y = array.view(-1, 1).expand((1, K, M))
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
